...
> This is an independently maintained HeyGe customization of the upstream
> [2dust/v2rayN](https://github.com/2dust/v2rayN) client. It is not affiliated
> with or endorsed by the upstream project. It remains licensed under GPL-3.0;
> see [LICENSE](LICENSE) and the complete source in this repository.

## HeyGe changes

- Adds **HeyGe 登录并同步**. Customers sign in with their HeyGe website account
  over HTTPS, then one managed subscription refreshes all delivered, unexpired
  VLESS nodes into one group. The client does not persist the website password.
- Replaces the upstream promotion destination with **跨境导航**, which opens
  <https://sxcrs.com> in the system browser.
- Leaves upstream proxy, subscription, routing and core-management behaviour
  unchanged.

The HeyGe web platform must expose the scoped desktop-login endpoint documented
in `docs/v2rayn-custom-client.md` in the platform repository before this custom
menu can be used.
